...
bool cAudioManager::LoadTextToSpeech() {
	...
	document.parse<0>(&buffer[0]);
	// Find the root node
	root_node = document.first_node("TTS");

	// Iterate through all of the root node's children
	// use to build audio object
	for (rapidxml::xml_node<>* platform_node = root_node->first_node("paragraph");
		platform_node;
		platform_node = platform_node->next_sibling()) {

		if (platform_node->first_attribute("sentence")->value() > 0) {
			std::string sTmp(platform_node->first_attribute("sentence")->value());
			std::wstring wsTmp(sTmp.begin(), sTmp.end());
			vec_TTS_Paragraph.push_back(wsTmp);
		}
	}
